    ...
    class Sbom < Vanagon::CLI
      DOCUMENTATION = <<~DOCOPT.freeze
        ...
        Options:
          ...
          -t, --type TYPE                  SBOM type: spdx or cyclonedx [default: cyclonedx]
          -f, --format FORMAT              Output format: json, yaml, or tag [default: json]
        ...
        Project-Name:
          May be a project name of a project from the configs/projects directory or 'all' to generate bill of materials for all projects.
        Platforms:
          May be a platform name of a platform from the configs/platforms directory or 'all' to generate bill of materials for all platforms.
      DOCOPT
      ...
      def run(options)
        ...
        sbom_type = options[:type].to_sym
        sbom_format = options[:format].to_sym
        failures = []
        ...
        unless failures.empty?
          VanagonLogger.info "Failed to generate bill of materials for the following:"
          failures.each do |failure|
            VanagonLogger.info failure
          end
        end

        VanagonLogger.info "Finished generating bill of materials"
      end
